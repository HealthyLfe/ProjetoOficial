 ...
 */
class EditarAlmocoFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view: View = inflater.inflate(R.layout.fragment_editar_almoco, container, false)

        infoReceita()

        val btnCancelar = view.findViewById<View>(R.id.can_edit_rec_almoco) as Button
        ...
        val btnUpload = view.findViewById<View>(R.id.btnUpload_editar_almoco) as Button
        ...
        val btnSalvar = view.findViewById<View>(R.id.btn_edit_rec_almoco) as Button
    ...
    private fun infoReceita() {
        ...
        ref.addValueEventListener(object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot?) {
                ...
                txt_titulo_edit_almoco.setText(rec?.titulo)
                txt_titulo_edit_almoco.setSelection(txt_titulo_edit_almoco.text.length)

                txt_ingred_edit_almoco.setText(rec?.ingrediente)
                txt_ingred_edit_almoco.setSelection(txt_ingred_edit_almoco.text.length)

                txt_descricao_edit_almoco.setText(rec?.desc)
                txt_descricao_edit_almoco.setSelection(txt_descricao_edit_almoco.text.length)

                Picasso.get().load(rec?.img).into(img_edit_almoco)
    ...
    private fun editar() {
        val titulo = txt_titulo_edit_almoco.text.toString().trim()
        val ingrediente = txt_ingred_edit_almoco.text.toString().trim()
        val desc = txt_descricao_edit_almoco.text.toString().trim()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 0 && resultCode == Activity.RESULT_OK && data != null) {
            ...
            img_edit_almoco.setImageBitmap(imgNova)

            btnUpload_editar_almoco.alpha = 0f
